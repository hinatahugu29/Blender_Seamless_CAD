...
1. 打开 **Modify & Pattern**，按下 **Fillet**
2. 选择模式会自动开启，并且已经切换到选边状态
3. 在视图中点击需要的边——它们会变成橙色并保持选中
4. 在 **Active Property Editor** 中设置半径

**先添加修改器，再选取。** 选择结果保存在 Fillet *内部*，所以必须先有 Fillet，
选择才有地方存放。如果在普通形状处于活动状态时进入选择模式，点击不会选中
任何东西——现在插件会给出提示，而不是默默忽略这次点击。

> **提示：** 在选择模式下按住 <kbd>Alt</kbd>，可以不退出该模式直接使用 Blender
> 的常规操作控制器。按住 Shift 点击可以追加选择，而不是替换。
