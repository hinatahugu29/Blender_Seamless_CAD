...
（※2026-08-17に受領したお客様からの質問・要望は [`CUSTOMER_REQUESTS_20260817.md`](CUSTOMER_REQUESTS_20260817.md) を参照）
...
角度拘束で実際に踏んだ。`LinesAtAngle` の残差は外積なので **+90度と -90度を
区別しない**。符号をわざと反転させても、90度のテストは**通ってしまった**。
30度のケースを入れていたおかげで捕まった。

一般則として、**その拘束が対称性を持つ値（90度、正方形、原点対称の配置）だけで
テストを書くと、符号や向きの誤りが素通りする。**必ず非対称な値を1つ混ぜること。
[[headless-regression-test]] の「1軸だけで判断しない」と同じ性質の罠。
...
> **訂正 (2026-08-17)**: 「エクスポート側にスケール指定が無い」は**解消済み**。
> [`occ_step.cpp:121`](Blender_CAD_V_8_1_5_1/src_rust/src/occ_step.cpp:121) の
> `export_stack_to_step` が `scale` を取り、`gp_GTrsf` を形状側に掛けている
> (`write.step.unit` は「同じ大きさに別の単位名を付ける」だけなので使っていない)。
> **残っているのは名前・色・アセンブリ構造のほうだけ。**
>
> なお現在の実装は**スタック全体を1形状として書き出している**。Part ごとに
> 名前を付けるには書き出しの粒度そのものを変える必要があり、ここが
> 「`STEPCAFControl_Writer` への載せ替え」で済まない部分。

---

## 5. 計測がない → **完了（2026-08-17 に確認）**

> **この節は解決済み。** 8.1.5.7 の時点で
> [`ui/ui_main_panel.py:118`](Blender_CAD_V_8_1_5_1/CAD_8_1_5_1/ui/ui_main_panel.py:118) に
> `SEAMLESS_PT_MeasurePanel` が存在する。
>
> - **Part 全体**: 体積・表面積・寸法 (X/Y/Z)・重心。`seamless.measure_part`
> - **辺/面の個別**: 長さ・面積・半径。`seamless.measure_selected`
> - 測るのは**ボタンを押したときだけ**。`draw()` からカーネルを呼ばない
>   （「パネルを開いているだけで重い」を避けるため）
> - lineage が現在の形状に一致しないときは**値を出さずに再選択を促す**
> - モディファイアの対象を選び直している最中は「適用前の形を測っている」と警告する
> - 体積 0 を「測定失敗」ではなく「閉じたソリッドではない」と表示し分ける
>
> カーネル側は `measure_stack` / `measure_entity`
> ([`occ_core.cpp:2041`](Blender_CAD_V_8_1_5_1/src_rust/src/occ_core.cpp:2041))。
> 以下は着手前の記録として残す。
...
## 6. Face Inset が大きい距離で破綻する ← 未解決の不具合（穴ではなく、バグ）

### 事実

2026-09-05 に Linux の利用者から報告。sphere + cylinder + fillet の上面に
Face Inset をかけ、距離を上げていくと落ちる。調査で分かったことは
`reply_to_reporter-20260905-1.md` / `-20260906-1.md` / `-20260906-2.md` にある
（報告者へ送った返信そのもの。**経緯の一次資料。`.gitignore` 対象で手元にしか無い** —
公開向けに書かれた文章ではないので追跡しない。技術的に生きている事実は下に写してある。
`LISTING_PREP.md` と同じ扱い）。

- **Linux / macOS**：カーネルごと落ちていた。原因は `OCC_CONVERT_SIGNALS` 未定義で
  `OCC_CATCH_SIGNALS` が13箇所すべて空マクロだったこと（8.1.5.12 で修正、
  [[occ-catch-signals-unix-nop]]）。**ただし直したのは「落ちなくなった」ところまで。**
  faultを捕捉してフォールバックしているだけで、faultそのものは残っている
- **Windows**：落ちない。代わりに**形が壊れる**。フィレットが消えることがある。
  再現の録画が `broken/broken.webm`（2026-09-05、5.1MB。`.gitignore` 対象）。
  最終フレームの `3_FACE_INSET_Seamless_Proxy` で形が明らかに誤っている

### 落ちる場所と、フォールバックが何をしているか（2026-09-14 に読んで確認）

落ちるのは [`occ_modifiers.cpp`](Blender_CAD_V_8_1_5_1/src_rust/src/occ_modifiers.cpp:1393) の
`BRepOffsetAPI_MakeOffset(f, GeomAbs_Arc)` → `Perform(-inset_dist)`。
**面の境界ワイヤを内側へオフセットする処理**で、内側へ寄せると辺が消え、角が自己交差し、
円弧が潰れる。OCCT はその過程でヌルハンドルを触ることがある
（報告者のログの `SIGSEGV ... Address 20` の 0x20 は、ヌル + 構造体オフセットの典型）。

**捕まえた後のフォールバックは「オフセット」ではない。**

```
R     = sqrt(面積 / π)        ← 面を「同じ面積の円」とみなす
scale = (R - inset_dist) / R
面を重心中心に scale 倍する
```

- **円なら厳密に正しい。** 報告者のモデルは円柱の上面だったので、これで正解になる
- **円でなければ正しくない。** 真のオフセットは全ての辺から等距離に寄せるが、
  相似縮小は**重心から遠い辺ほど大きく動く**。細長い長方形なら短辺と長辺で寄り方が違う

### 実測（2026-09-14）— Windows と Linux の結果は一致している

同じ形・同じ値で両プラットフォームの体積と表面積を測り、**比較できた11件すべてが
小数6桁まで一致**した（円柱の上面 inset=0.3/0.4/0.45、箱の上面 inset=0.01〜0.45 の7点。
Windows は Blender 5.2.1、Linux は配布 ZIP + Blender 5.1.1）。

9/6 の手紙に「**形が正しいかは Blender で確認できていない**」と書いて残した宿題は、
**この範囲では答えが出た**。単純な面では MakeOffset が両方で成功していて、
フォールバックの出番が無いのだと思われる。

**未検証:** そう「思われる」だけで、確かめていない。この実行でどちらの経路を通ったかを
ログで確認しようとしたが、カーネルは別プロセスで stderr が自分のログへ行くため、
この計測では取れなかった。**フォールバックが実際に使われ、かつ円でない面だったケースは
一度も測っていない。** そこが本当に問題になるかは不明。

### なぜここに書くか

**8.1.5.12 で「直した」と記憶されやすいが、直っていない。** 落ちなくなっただけ。
`docs/*/limitations.md` に利用者向けの記述を足した（2026-09-14）が、
原因側は手つかず。**落ちないことと、正しいことは別。**

### 着手するなら

`apply_face_inset` の内側オフセット（面の境界を内側へ送る処理）。
**Windows では「できない」と報告して終わるはずの経路で、壊れた形を返している。**
まず「距離いくつで破綻するか」を非対称なモデルで数値として押さえること
（対称な値だけで確かめない — 下記のテストの作り方を参照）。
...
```
2 (スケッチ寸法拘束)  ← 完了 (8.1.5.6)
  ↓
5 (計測)              ← 完了 (8.1.5.7)
  ↓
3 (Hole)              ← 中コスト・高頻度
  ↓
4 (STEP メタデータ)   ← 看板の残り半分。スケール指定の部分は完了済み
  ↓
1 (Tree への挿入)     ← 一番大きい。lineage の設計と正面から向き合う
```

> **2026-08-17 追記**: 2 と 5 が片付いたため、本書の残りは 3 / 4 / 1 の3件。
> そこへ「他社にあってこれに無いもの」を突き合わせた結果が
> `COMPETITIVE_GAP_ANALYSIS.md`、着手順に落としたものが
> `IMPLEMENTATION_ROADMAP.md`。**実装に入るなら後者を見ること。**
> 本書は「自分のコードを読んで見つけた穴」という別の視点として残す。
