...
### 4-11. モディファイア/パターン系の調査(2026-08-01) — 不具合なし
プリミティブ以外も一通り当たった。**結論: いずれも正常**。ただし全部が最初は
「効いていない」ように見えたので、そのからくりを残しておく。

| 型 | 一見 | 実際 |
|---|---|---|
| FILLET / CHAMFER | 追加しても何も起きない | **対象エッジが空だから**。指定すれば 8→124 / 8→12 頂点 |
| DRAFT / FACE_OFFSET / FACE_INSET | 同上 | 同上(対象面が要る) |
| SHELL | 効く | 対象なしでも全体に効く |
| MIRROR | 効かない | **原点対称の箱を原点で鏡像化すると重なるだけ**。x=2 へ寄せれば 8→16 頂点 |
| ARRAY_LINEAR | distance が効かない | **既定の軸が Z なのに X 幅だけ見ていた**。Z 幅は 4.0→7.0→13.0 と正しく変化 |
| ARRAY_CIRCULAR | 効く | count / distance とも有効 |
| REVOLVE | 効かない | 2D プロファイルが要る。箱を渡しても回らないのは正しい |
| CLEANUP | 効かない | 素の箱には統合すべき面が無い(仕様。[[cad-unify-cleanup-modifier-design]] 参照) |

**教訓が2つ**:
- 対象(`target_lineages` / `target_uuid`)を要する型は、空のまま試すと必ず
  「壊れている」ように見える。まず対象を与えること。
- **1軸だけ見て判断しない。** ARRAY_LINEAR は既定 Z 方向なので X 幅は変わらない。
  ブーリアン誤検知(§4-8 の注記)と同じ型の失敗を2度踏んだ。

FILLET と CHAMFER は回帰テストに取り込んだ(§7)。残りは対象の準備が重いか
2Dプロファイルが要るため、当面は手動確認に残す。

### 4-12. CLEANUP が誤差で辺の統合を拒否していた ✅ 修正済(2026-08-01)
**症状**: 穴 → 面オフセットで貫通 → CLEANUP、の後で
**面は結合されているのに、一直線に並ぶ2本の辺が1本にならない**(利用者報告)。

**原因**: `occ_core.cpp` の CLEANUP 分岐が `ShapeUpgrade_UnifySameDomain` を
**許容誤差を指定せずに**構築していたため、OCCT の既定値が効いていた。

| | 既定値 |
|---|---|
| 角度許容誤差 | `Precision::Angular()` = **1e-12 rad**(約 6e-11 度) |
| 線形許容誤差 | `Precision::Confusion()` = 1e-7 |

ヘッダに「この値より大きい角度で接続された形状は統合されない」と明記されている。
ブーリアン・オフセット・フィレットを通った形状が小数点以下11桁まで一直線であり続ける
ことはないので、**統合は浮動小数点の誤差だけを理由に拒否されていた**。

**旧バイナリを対照にした実測**(2つの箱を融合し、片方をわずかに傾ける):

| 傾き | 修正前 | 修正後 |
|---|---|---|
| 0° | 面 10→6 / 辺 20→12 | 同じ |
| **1e-9°** | **面 10→8 / 辺 20→18** | 面 10→6 / 辺 20→12 |
| 1e-5° | 面 10→8 / 辺 20→18 | 面 10→6 / 辺 20→12 |
| 1e-4° | 面 10→8 / 辺 20→18 | 同じ(正しく拒否) |

10億分の1度で面が一部しか統合されず辺が6本残る。しかも**失敗の形が
「面は結合されるのに辺が残る」**で、報告と一致していた。

**修正**: `SetAngularTolerance(1e-6)` / `SetLinearTolerance(1e-6)`。意図的な角とは
3桁の開きがあり(最も浅いドラフト角 0.1度 = 1.7e-3 rad)、上表 1e-4° の行が
「必要な場面では今も拒否する」ことを示している。**利用者の実機で解消を確認済み。**

**未修正**: CLEANUP パネルの `unify_faces` / `unify_edges` チェックボックスは効かない。
Python は送り Rust の構造体も持つが、C++ 呼び出しが `(mod_c, true, true, true)` の
べた書き。`cargo build` が "fields `unify_faces` and `unify_edges` are never read" と
警告している。両方常に ON なので上記の原因ではないが、UI にあって効かない欄が2つ残る。
配線には FFI シグネチャの変更が要る。
