...
### 4-6. 背景実行の Undo で Blender ごと落ちる ★要調査(2026-07-30 発見)
出品前チェックの自動化中に見つかった。**アドオンを register した状態で
`bpy.ops.ed.undo()` を呼ぶと、`EXCEPTION_ACCESS_VIOLATION` で Blender ごと死ぬ**
ことがある(`python313.dll`、終了コード 11)。

分かっていること:

- ある再現スクリプトでは **5回中5回**落ちる。決定的。
- `bpy.app.handlers.undo_post` / `redo_post` を空にすると**落ちず、Undo は正しく
  2件→1件に戻る**。つまり `undo_redo_post_handler` が絡んでいる。
- ところが同じハンドラを `clear()` してから登録し直すと落ちない。ほぼ同じ手順の
  別スクリプトでも落ちない。**条件が特定できていない。**
- undo の前に4秒待っても落ちる。単純な非同期処理のレースではない。
- 背景実行では `ed.undo` は最初 poll が False で、`ed.undo_push` を**3回**入れると
  通るようになる(1回では足りない)。

**未確認: これが GUI の Ctrl+Z にも起きるのかどうか。** 背景実行特有かもしれないし、
実ユーザにも起きる致命傷かもしれない。**出品前に実機で Ctrl+Z を必ず確認すること。**

回帰テストには入れていない。落ちるテストは失敗を報告せず、残りのテストを道連れに
してスイート全体を無意味にするため(§7)。
