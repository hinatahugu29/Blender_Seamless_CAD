...
| ファイル | 内容 |
|---|---|
| `CLAUDE.md` | **作業前に読む要約。** 編集して良い場所、ビルドの鉄則、既知の罠、ドキュメントの権威分類 |
...
| `INVENTORY_V8_1_5_1.md` | **今何があるか。** プリミティブ・オペレータ・パネル・同梱物の実測棚卸し |
| `FEATURE_GAPS.md` | 自分のコードを読んで見つけた穴 |
| `COMPETITIVE_GAP_ANALYSIS.md` | Fusion 360 / Plasticity との比較。何が届き、何を諦めるか |
| `IMPLEMENTATION_ROADMAP.md` | **これから何をするか。** 着手順と、各項目の触る場所・罠 |

> root には `.md` が25本あり、**半分以上は歴史的資料**です。各ファイルの冒頭に
> 状態バナーを付けてあります。分類の一覧は `CLAUDE.md` §6 にあります。
> 版番号は `V1.2.0`〜`V1.4.0` → `V2.0.4` → `V8.1.x` と**3世代混在**しているので、
> 古い計画書の版番号を現行のものと解釈しないでください。
