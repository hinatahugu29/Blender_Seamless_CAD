...
対象: `Blender_CAD_V_8_1_5_1/CAD_8_1_5_1` (bl_info version 8.1.5.8, target Blender 4.2.0 以上)
作成日: 2026-07-15 / 全面検証: 2026-08-02 / **数値再実測: 2026-08-14 (8.1.5.7)**

> **本文の数値は 8.1.5.7 時点の実測値のまま。** 8.1.5.8 では数え直していない。
> 8.1.5.7 → 8.1.5.8 の差分はモーダルのイベント透過・描画色・スケッチのスナップ
> トグル追加（プロパティ1つ、UI 行1つ）で、オペレータ数・パネル数・
> プリミティブ型数は変わっていないはずだが、**再計測はしていない**。
...
| **IGES** (`.igs` / `.iges`) | — | ✅ `export_iges` **[2026-08-17 追加]** (幾何のみ) |
...
**IGES は 2026-08-17 に対応した**(書き出しのみ、幾何のみ)。
それ以前は非対応で、販売ページの原稿に「STEP & IGES 対応」と書かれていたのは
誤りだった(2026-08-01 に全体を検索して確認し、原稿を修正した経緯がある)。
**実装したのは誤記を正当化するためではない** — STEP/STL と同じ層に収まり、
同梱済みの `TKDEIGES.dll` を配線するだけだったため。
読み込みは今も非対応。
...
| 仕組み | 内容 |
|---|---|
| `regression_test.py` | ヘッドレス回帰テスト **44件** (2026-08-17 実測)。Blender 4.2 / 5.1 で確認。登録、プリミティブ追加、双方向同期、削除同期、確定フェーズの契約、FILLET/CHAMFER が実際に形状を変えること、スケッチ拘束ソルバ、スケッチ確定、ベイク、STEP 書出、保存→再読込、Undo 1回=1ステップ |
