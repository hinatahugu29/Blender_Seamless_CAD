...
> **8.1.5.16**（2026-09-14 時点。正確な値は `CAD_8_1_5_1/__init__.py` を見ること）。ディレクトリ名は版が上がっても変えていない。
...
8.1.5.16 からは回帰テストの1本目（`kernel reports its build`）がこれを検出する。

rustc の版は `src_rust/rust-toolchain.toml` で **1.98.1 に固定**してある。
固定前は、カーネルのソースを変えていないのに CI のビルドごとにバイナリの
サイズが動いていた（8.1.5.12→8.1.5.15 で 1,456 バイト。差は rustc の
ソースパスハッシュだけで、C++ 側は同一）。**上げるときは意識して上げ、
3プラットフォームを作り直すこと。**
...
| 罠 | 症状 | 対処 |
|---|---|---|
| **古い `cad_server` が残る** | 新しいアクションが `None` を返す。**8.1.5.16 以降は版の不一致をコンソールに出す**（それ以前は無言） | ポート 8080 のプロセスを落としてから起動し直す。カーネルを変えたら `main.rs` の `KERNEL_BUILD` と `core_bridge._EXPECTED_KERNEL_BUILD` を**両方**上げる |
...
`HANDOVER_20260914.md`（8.1.5.16 の配布までの記録。**2026-09-14 に配布済み**。
残作業は全て消化されているので、計画の根拠には使わない）／
